...
-- EMP 테이블에서 직원의 이름(emp_name)을 모두 대문자, 소문자, 이름 글자수를 조회
	select upper(emp_name) upper_data, lower(emp_name), CHAR_LENGTH(emp_name) len from emp  

-- 직원 이름(emp_name) 의 자릿수를 15자리로 맞추고 15자가 안되는 이름의 경우  공백을 앞에 붙여 조회. 
	select lpad(emp_name, 15, ' ') name from emp 
...
************************************************************************** */
	...
	select round(3.123456); -- 정수반환 
	select round(3.123456, 2); -- 소수점 2째자리까지 반환
	select round(32.123456, -1); -- 정수로 넘어
...
-- (단, 15% 인상된 급여는 올림해서 정수로 표시하고, 별칭을 "SAL_RAISE"로 지정.)
	select emp_id, emp_name, salary, salary * 1.15 "SAL_RAISE" from emp;


-- 위의 SQL문에서 인상 급여(sal_raise)와 급여(salary) 간의 차액을 추가로 조회 
-- (직원ID(emp_id), 이름(emp_name), 15% 인상급여, 인상된 급여와 기존 급여(salary)와 차액)
	
	select emp_id, emp_name, salary, salary * 1.15 "SAL_RAISE", (salary*1.15 - salary) diff from emp;
...
-- EMP 테이블에서 부서이름(dept_name)이 'IT'인 직원들의 '입사일(hire_date)로 부터 10일전', 입사일, '입사일로 부터 10일 후' 의 날짜를 조회. 
	select ADDDATE(hire_date, INTERVAL -10 day), hire_date,  ADDDATE(hire_date, INTERVAL 10 day) from emp where dept_name = 'IT';

-- 부서가 'Purchasing' 인 직원의 이름(emp_name), 입사 6개월전과 입사일(hire_date), 6개월후 날짜를 조회.
	select emp_name, ADDDATE(hire_date, INTERVAL -6 month), hire_date, ADDDATE(hire_date, INTERVAL 6 month) from emp where dept_name = 'Purchasing'; 
...
--  각 직원의 이름(emp_name), 근무 개월수 (입사일에서 현재까지의 달 수)를 계산하여 조회. 근무개월수 내림차순으로 정렬.
	select emp_name, ceil(DATEDIFF(CURDATE(), hire_date)/30) day from emp order by  ceil(DATEDIFF(CURDATE(), hire_date)/30) desc;	
	select emp_name, timestampdiff(month, hire_date, curdate()) as '근무개월' from emp order by 2 desc; 

-- 250개월 이상 근무한 직원 정보를 조회
	select * from emp where timestampdiff(month, hire_date, curdate()) >= 250
...
-- EMP 테이블에서 직원의 ID(emp_id), 이름(emp_name), 업무(job), 부서(dept_name)을 조회. 부서가 없는 경우 '부서미배치'를 출력.
	select emp_id, emp_name, job, ifnull(dept_name, '부서미배치') from emp; 
...
--		when salary < 10000 then '2등급'end from emp;
		else '2등급' end from emp;
	
	select salary, if(salary >= 10000, '1등급', '2등급') from emp
...
-- 업무(job)가 'AD_PRES'는 '대표', 'FI_ACCOUNT'는 '회계', 'PU_CLERK'의 경우 '구매'가 출력되도록 조회
	select emp_id, emp_name, 
		...
		when 'PU_CLERK' then '구매' end from emp where job in ('AD_PRES', 'FI_ACCOUNT', 'PU_CLERK');
...
-- 급여 인상분은 부서이름이 'IT' 이면 급여(salary)에 10%를 'Shipping' 이면 급여(salary)의 20%를 'Finance'이면 30%를 나머지는 0을 출력
	select dept_name, 
		...
		else 0 end as rise_salary from emp
...
-- 단 급여 인상율은 급여가 5000 미만은 30%, 5000이상 10000 미만는 20% 10000 이상은 10% 로 한다.
	...
	when salary >= 10000 then salary * 0.1 end from emp;
