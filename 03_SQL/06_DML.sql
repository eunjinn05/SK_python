...
-- 직원 ID가 200인 직원의 급여를 10% 인상한 값으로 변경.
	UPDATE emp set salary = salary * 1.10 where emp_id = 200
...
-- 부서 ID가 100인 직원의 커미션 비율을 0.2로 salary는 3000을 더한 값으로, 상사_id는 100 변경.
	update emp set comm_pct = 0.2, salary = salary + 3000, mgr_id = 100 where emp_id = 100

--  IT 부서의 직원들의 급여를 3배 인상
	update emp set salary = salary * 3 where job_id = 'IT_PROG'

-- EMP 테이블의 모든 데이터를 MGR_ID는 NULL로 HIRE_DATE 는 현재일시로 COMM_PCT는 0.5로 수정.
	update emp set mgr_id = null, hire_date = DATE_FORMAT(now(),'%Y-%m-%d'), comm_pct = 0.5
