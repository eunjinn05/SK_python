...
-- EMP 테이블의 직원 ID(emp_id), 직원 이름(emp_name), 업무(job) 컬럼의 값을 조회.
	SELECT emp_id, emp_name, job from emp e
...
-- EMP 테이블에서 emp_id는 직원ID, emp_name은 직원이름, hire_date는 입사일, salary는 급여, dept_name은 소속부서 별칭으로 조회결과를 출력 한다.
	SELECT emp_id, emp_name, hire_date, salary, dept_name as dep from emp 
...
-- EMP 테이블에서 직원의 이름(emp_name), 급여(salary) 그리고  급여 + 1000 한 값을 조회.
 	select emp_name, dept_name, salary+1000 from emp
...
-- EMP 테이블에서 'Sales' 부서에 속하지 않은 직원들의 ID(emp_id), 이름(emp_name),  부서명(dept_name)을 조회.
	SELECT emp_name, dept_name from emp where dept_name != 'Sales'
...
-- EMP 테이블에서 커미션비율(comm_pct)이 0.2~0.3 사이인 직원의 ID(emp_id), 이름(emp_name), 커미션비율(comm_pct)을 조회.
	select emp_id, emp_name, comm_pct from emp where comm_pct >= 0.2 and comm_pct <= 0.3
...
-- 급여(salary)가 $4,000에서 $8,000 사이에 포함된 직원들의 ID(emp_id), 이름(emp_name)과 급여(salary)를 조회
	select emp_id, emp_name, salary from emp where salary  >= 4000 and salary <= 8000


-- EMP 테이블에서 2004년에 입사한 직원들의 ID(emp_id), 이름(emp_name), 입사일(hire_date)을 조회.
-- 참고: date/datatime에서 년도만 추출: year(컬럼명)
	select emp_id, emp_name, hire_date from emp where hire_date >= '2004-01-01' and hire_date <= '2004-12-31'
...
-- EMP 테이블에서 'Sales' 와 'IT', 'Shipping' 부서(dept_name)가 아닌 직원들의 모든 정보를 조회.
	select * from emp where dept_name not in ('Sale', 'IT', 'Shipping')
...
-- EMP 테이블에서 업무(job)에 'MAN'이 들어가는 직원들 중에서 Salary가 10,000 이상이거나 2008년 이후 입사한 직원들 정보 조회
	select * from emp where job like '%MAN%' and (salary >= 10000 or YEAR(hire_date) >= 2008)
...
--  업무(job) 순서대로 (A -> Z) 조회하고 업무(job)가 같은 직원들은 급여(salary)가 높은 순서대로 2차 정렬해서 조회.
	select emp_id, emp_name, job, salary from emp order by job asc, salary desc
